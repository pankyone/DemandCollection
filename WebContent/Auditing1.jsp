...
String depsql = "";
String addsql = "";
depsql = "select department from UserPeople where username = '" + user + "'";

ResultSet deprs = db.executeQuery(depsql);

if(deprs.next())
{
	part = deprs.getString(1);
}
String sql="";
if(oldtext=="")
{
	sql="select * from JiGou join XuQiu on JiGou.code = XuQiu.code"+
			" where xqstatus2=0 and xqstatus1=1 and xqdep = '"+part+"'";
}
else
{
	addsql=" and " + oldsearch + " like '%" + oldtext + "%'";
	sql="select * from JiGou join XuQiu on JiGou.code = XuQiu.code"+
			" where xqstatus2=0 and xqstatus1=1 and xqdep = '"+part+"'" + addsql;
}
