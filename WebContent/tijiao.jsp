...
<jsp:useBean id = "db" class = "bean.JBean" scope = "page" >
	<%
		...
		String NeedSQL = "Insert Into XuQiu Values('" + code + "','" + xqneedname + "','" + xqtime_before + "','" 
				+ xqtime_after + "','" + xqsummary1 + "','" + xqsummary2 + "','" + xqsummary3 + "','" 
				+ xqkeywords + "','" + xqstudy_type + "','" + xqclassification + "','" + xqtec_field + "','" + xqinstruction + "','"
				+ xqtechnologyapplication + "','" + xqcoo_model + "','" + xqcooperative +  "','" + xqself_collected + "'," 
				+ xqstatus1 + ",'',"+xqstatus2+",'','')";
		...
		if (i==1&&j==1){
			...
			if(xqstatus1==2)
			{
			out.println("<script language='javaScript'> alert('保存成功，可随时修改。');</script>");
			response.setHeader("refresh", "0;url = Index.jsp");
			}
		}
		else
